...
WAVStreamer::WAVStreamer(char *file, float duration, int index)
...
WAVStreamer::WAVStreamer(QList<QVariantList> valLs, int index)
...
void *WAVStreamer::streamingThread(void* p)
{
	return ((WAVStreamer *)p)->streaming();
}


void *WAVStreamer::streaming()
{
...
WAVStreamer* WAVStreamer::createNew(char * file, float duration, int index)
{
	return new WAVStreamer(file, duration, index);
}	

WAVStreamer* WAVStreamer::createNew( QList<QVariantList> valLs, int index)
{
    return new WAVStreamer(valLs, index);
}


void WAVStreamer::afterPlaying(void* clientData)
{
	//  reach the end of current media file 
	WAVStreamer *pStream = (WAVStreamer *)clientData;
...
void WAVStreamer::myAfterPlaying ()
{
   // qDebug() << __func__;
        fAudioSink->stopPlaying();
...
int WAVStreamer::continuePlay()
{


}

void WAVStreamer::pause()
{
	if (fAudioSource) {
		fAudioSink->stopPlaying();
...
//	*env << bitsPerSecond << " bits-per-second\n";
	...
	ServerMediaSession* sms
		...
		"Session streamed by \"WAVStreamer\"", True/*SSM*/);
