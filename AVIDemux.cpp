...
{
    fFormatCtx = NULL;
    //if(av_open_input_file(&fFormatCtx, fInputFileName, NULL, 0, NULL) != 0)
	...
	//  print file information
      av_dump_format(fFormatCtx, 0, fInputFileName, 0);
      fflush(stdout);
	...
	for(unsigned int i = 0; i < fFormatCtx->nb_streams; i++) {
        ...
        else if(fFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO){
			fAudioIndex = i;
		}
...
int AVIDemux::seekStream(float pos)
{
    ...
    start_pos = /*url_ftell*/ avio_tell(fFormatCtx->pb);
				...
				if (fVideo_diff > 0) {  //  we should save some video packets until audio pts 
					...
					if (packet.stream_index == fAudioIndex) {
						...
						if (convertPTS(&packet) < convertPTS(video_pkt))  {
                            ...
                            /*url_fseek*/avio_seek(fFormatCtx->pb, start_pos, SEEK_SET);
...
unsigned int AVIDemux::getBitrate(char streamIndex)
{
	...
	if (streamIndex == fVideoIndex) {
		if(fAudioIndex >= 0) {
			AVStream* audio = fFormatCtx->streams[fAudioIndex];
            return (fFormatCtx->bit_rate  /* - audio->codecpar->bit_rate tootzoe */ ) / 1000;//  ?? 1024
		}
		else {
	...
	if (streamIndex == fAudioIndex) {
		AVStream* audio = fFormatCtx->streams[fAudioIndex];
        int bits_per_sample = av_get_bits_per_sample(audio->codecpar->codec_id);
        return (bits_per_sample ? audio->codecpar->sample_rate * audio->codecpar->channels * 2 : audio->codecpar->bit_rate) / 1000; // ??
	}
...
//	OutputDebugStringA(buf);
	...
	if (afterGettingFunc != NULL) {
		...
		if (fVideoIndex == streamIndex) {		
            ...
            AVCodecParameters *codecpar = fFormatCtx->streams[fAudioIndex]->codecpar;
			(*afterGettingFunc)(afterGettingClientData,
				totNumBytesCopied, 
				0,
				presentationTime,
                codecpar->frame_size * 1000000L / codecpar->sample_rate);
		}
	}
	return True;
}
...
static void fixH264Bsfc(AVCodecContext *codec, AVBitStreamFilterContext *h264bsfc, AVPacket *packet)
{
									  ...
									  void* onCloseClientData) 
{
    ...
    do {
        ...
        int ret ;
        if ( (ret = av_read_frame(fFormatCtx, &packet)) < 0) {

            qDebug() << " av_read_frame not == 0 " << "  reason: " << av_err2str(ret) ;
        ...
        if((fVideoIndex == acquiredStreamIndex) && (getCodecID(fVideoIndex) == AV_CODEC_ID_H264))
        {
            if(!h264bsfc)
                h264bsfc = av_bitstream_filter_init("h264_mp4toannexb");

            fixH264Bsfc(fFormatCtx->streams[streamIndex]->codec, h264bsfc, &packet);
        }


     //   qDebug() << " readindex: " << acquiredStreamIndex  << "  want: " << quint8( streamIndex) ;
...
void AVIDemux::sendVideoFrame(unsigned char*  , unsigned size)
{
	struct timeval presentationTime;
	struct OutputDescriptor& out = fOutput[0];
	int frameDuration = 0;
